...
export default function Board({isUserFirst, setShowBoard, handleGameScore}: BoardProps) {
    ...
    const handleOnPressSquare = (index: number) => {
      ...
      // update board with user turn
       ...
       if (!gameOver) {
        const opponentGuess = calculateOpponentTurn(boardCopy);
        boardCopy[opponentGuess] = 'O';
        setBoard(boardCopy);
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.row}>
            <Square value={board[0]} handleOnPress={() => handleOnPressSquare(0)} disabled={gameOver}/>
            <Square value={board[1]} handleOnPress={() => handleOnPressSquare(1)} disabled={gameOver}/>
            <Square value={board[2]} handleOnPress={() => handleOnPressSquare(2)} disabled={gameOver}/>
        </View>
        <View style={styles.row}>
            <Square value={board[3]} handleOnPress={() => handleOnPressSquare(3)} disabled={gameOver}/>
            <Square value={board[4]} handleOnPress={() => handleOnPressSquare(4)} disabled={gameOver}/>
            <Square value={board[5]} handleOnPress={() => handleOnPressSquare(5)} disabled={gameOver}/>
        </View>
        <View style={styles.row}>
            <Square value={board[6]} handleOnPress={() => handleOnPressSquare(6)} disabled={gameOver}/>
            <Square value={board[7]} handleOnPress={() => handleOnPressSquare(7)} disabled={gameOver}/>
            <Square value={board[8]} handleOnPress={() => handleOnPressSquare(8)} disabled={gameOver}/>
        </View>
    </View>
  )
}
